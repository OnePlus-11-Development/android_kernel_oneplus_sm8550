		...
		struct device_node *parent)
{
	...
	int feature_code = socinfo_get_feature_code();
	int pcode;
	u32 soc_code;

	feature_code = max_t(int, feature_code, 0);
	pcode = (feature_code >= SOCINFO_FC_Y0 && feature_code < SOCINFO_FC_INT_RESERVE) ?
		max_t(int, socinfo_get_pcode(), 0) : 0;

	soc_code = (pcode << 8) | feature_code;
	...
	dev_err(&device->pdev->dev,
		"No match for speed_bin: %d soc_code: %d\n",
